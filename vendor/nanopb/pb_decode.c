...
#endif // ifndef PB_INTERN_STRINGS

#ifdef PB_INTERN_STRINGS
#ifndef PB_ENABLE_MALLOC
#error "PB_INTERN_STRINGS requires PB_ENABLE_MALLOC"
#endif
#ifndef PB_BUFFER_ONLY
#error "PB_INTERN_STRINGS requires PB_BUFFER_ONLY to be enabled"
#endif
static bool checkreturn pb_dec_string(pb_istream_t *stream, const pb_field_iter_t *field)
{
    uint32_t size;
    ...
    if (PB_ATYPE(field->type) == PB_ATYPE_POINTER)
    {
        ...
        dest = mem_string_intern((const char*)stream->state, alloc_size);

        if (!dest)
            return false; // Failed to allocate memory

        *(void**)field->pData = dest;
        stream->state = (pb_byte_t*)stream->state + size;
